...
AMMobile_product_oneplus = Blueprint('AMMobile_product_oneplus', __name__)
api = Api(AMMobile_product_oneplus,  title='AM Mobile Product Oneplus',description='AM Mobile Product Oneplus API')
name_space = api.namespace('AMMobileProductOneplus',description='AM Mobile Product Oneplus')
...
class getProductNameAndMeta(Resource):
	...
	def post(self):
		...
		if brand == 'Oneplus':
			brand_id = 12
			try:
				get_product_name = get_product_name.replace(" Handset ", " ", 1)
				get_product_name = get_product_name.replace(" Handsets ", " ", 1)
				get_product_name = get_product_name.replace(" (150W)", " ", 1)
				get_product_name = get_product_name.replace(" PAC-MAN", " ", 1)
				get_product_name = get_product_name.replace(" (McLaren Edition)", " ", 1)
				get_product_name = get_product_name.replace(" One Plus ", " ", 1)

				product_name_split_for_storage_ram = get_product_name.split(' ')
				print(product_name_split_for_storage_ram)
				storage_ram_string = product_name_split_for_storage_ram[-1]
				
				if "+" in storage_ram_string:
					storage_ram_string_split = storage_ram_string.split('+')
					if "Gb" in storage_ram_string_split[0] or "GB" in storage_ram_string_split[0] or "gb" in storage_ram_string_split[0] or "gB" in storage_ram_string_split[0]:
						ram = storage_ram_string_split[0]	
					else:		
						ram = storage_ram_string_split[0]+" GB"
					storage = re.sub("[A-Za-z]+", lambda ele: " " + ele[0] + "", storage_ram_string_split[1])

					remove_ram_storage_string =  storage_ram_string_split[0]+"+"+storage_ram_string_split[1]										
					product_name = get_product_name.replace(remove_ram_storage_string,"",1).rstrip()

					print(storage)	
					print(ram)
					print(product_name)
				else:
					storage = re.sub("[A-Za-z]+", lambda ele: " " + ele[0] + "", product_name_split_for_storage_ram[-1])						
					ram = re.sub("[A-Za-z]+", lambda ele: " " + ele[0] + "", product_name_split_for_storage_ram[-2])

					remove_ram_storage_string =" "+product_name_split_for_storage_ram[-2]+" "+product_name_split_for_storage_ram[-1]
					product_name = get_product_name.replace(remove_ram_storage_string,"",1).rstrip()
				...
				if storage == '' or ram == '':
					storage = product_name_split_for_storage_ram[-3]+" GB"
					ram = product_name_split_for_storage_ram[-4]+" GB"
					remove_ram_storage_string =" "+product_name_split_for_storage_ram[-4]+" "+product_name_split_for_storage_ram[-3]+" GB "
					product_name = get_product_name.replace(remove_ram_storage_string,"",1).rstrip()
				...
				if get_storage_count > 0:
					...
					insert_storage_query = ("""INSERT INTO `meta_key_value_master`(`meta_key_id`,`meta_key_value`,`image`,`status`,`organisation_id`,`last_update_id`) 
						VALUES(%s,%s,%s,%s,%s,%s)""")
					insert_storage_data = (meta_key_id,meta_key_value,meta_key_value_image,meta_key_value_status,organisation_id,last_update_id)
					cursor.execute(insert_storage_query,insert_storage_data)
				...
				get_ram_data = (ram)
				get_ram_count = cursor.execute(get_ram_query,get_ram_data)
				...
				if get_ram_count > 0:
					...
					insert_ram_query = ("""INSERT INTO `meta_key_value_master`(`meta_key_id`,`meta_key_value`,`image`,`status`,`organisation_id`,`last_update_id`) 
						VALUES(%s,%s,%s,%s,%s,%s)""")
					insert_ram_data = (meta_key_id,meta_key_value,meta_key_value_image,meta_key_value_status,organisation_id,last_update_id)
					cursor.execute(insert_ram_query,insert_ram_data)
					ram_id = cursor.lastrowid
				...
				if get_product_count > 0:
					...
					if get_product_meta_count > 0:
						...
						image = "https://d1lwvo1ffrod0a.cloudfront.net/40941/10r-5g-le2608-oneplus-original-imageypzw8gumvyz.jpeg"
