...
AMMobile_product_nokia = Blueprint('AMMobile_product_nokia', __name__)
api = Api(AMMobile_product_nokia,  title='AM Mobile Product Nokia',description='AM Mobile Product Nokia API')
name_space = api.namespace('AMMobileProductNokia',description='AM Mobile Product Nokia')
		...
		if brand == 'Nokia':
			try:		
				...
				if "(" in storage_ram_string:					
					...
					remove_storage_ram_string = "("+ram+"+"+storage+")"
					product_name = get_product_name.replace(remove_storage_ram_string,"",1).rstrip()
					...
					storage_ram_string_split = storage_ram_string.split('/')
					if "Gb" in storage_ram_string_split[0] or "GB" in storage_ram_string_split[0] or "gb" in storage_ram_string_split[0] or "gB" in storage_ram_string_split[0]:
						ram = re.sub("[A-Za-z]+", lambda ele: " " + ele[0] + "", storage_ram_string_split[0])	
					else:		
						ram = storage_ram_string_split[0]+" GB"
					storage = re.sub("[A-Za-z]+", lambda ele: " " + ele[0] + "", storage_ram_string_split[1])
				...
				if get_product_count > 0:
					...
					if get_product_meta_count > 0:
						...
						image = "https://d1lwvo1ffrod0a.cloudfront.net/41181/nokia.jpeg"
