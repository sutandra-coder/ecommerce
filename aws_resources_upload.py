...
from awsconfige import ACCESS_KEY,SECRET_KEY
import requests
import boto3
import os
app = Flask(__name__)
...
aws_resources_upload = Blueprint('aws_resources_upload_api', __name__)
api = Api(aws_resources_upload,  title='MyElsa API',description='MyElsa API')
name_space = api.namespace('awsResourceUploadController',description='AWS Resources')
...
class uploadToS3BucketV2(Resource):
	def post(self,user_id):
		uploadURL = BASE_URL + 'aws_portal/awsResourceUploadController/uploadToS3Bucket/{}'.format(user_id)
...
class uploadToS3BucketV3(Resource):
	def post(self,user_id):
		...
		if uploadedfile:
			...
			return {"attributes": {"status": "success"},
				"responseList": [{
				  "FilePath": "https://d1o7xhcyswcoe3.cloudfront.net/{}".format(keyname),
				  "FileName": filename,
				  "FileSize": FileSize}],
				"responseDataTO": {}
				}
