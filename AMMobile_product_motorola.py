...
AMMobile_product_motorola = Blueprint('AMMobile_product_motorola', __name__)
api = Api(AMMobile_product_motorola,  title='AM Mobile Product Nokia',description='AM Mobile Product Motorola API')
name_space = api.namespace('AMMobileProductMotorola',description='AM Mobile Product Motorola')
	...
	def post(self):
		...
		if brand == 'Motorola':
			try:		
				...
				get_product_name = get_product_name.replace("(XT1770)", "", 1).rstrip()
				get_product_name = get_product_name.replace(" XT1795","",1)
				get_product_name = get_product_name.replace(" (XT1922)","",1)
				get_product_name = get_product_name.replace(" (Moto C)","",1)
				get_product_name = get_product_name.replace(" (Moto C Plus)","",1)
				get_product_name = get_product_name.replace(" XT1926","",1)
				product_name_split_for_storage_ram = get_product_name.split(' ')
				
				storage_ram_string = product_name_split_for_storage_ram[-1]
				print(storage_ram_string)
				...
				if "+" in storage_ram_string:
					...
					product_name_split_for_storage_ram = get_product_name.split(' ')
					storage = product_name_split_for_storage_ram[-2]+" GB"
					ram = product_name_split_for_storage_ram[-4]+" GB"
					remove_ram_storage_string =  " "+product_name_split_for_storage_ram[-4]+" GB "+product_name_split_for_storage_ram[-2]+" GB"
					#print(remove_ram_storage_string)
					print(ram)	
					print(storage)
					product_name = get_product_name.replace(remove_ram_storage_string,"",1).rstrip()
					print(product_name)
				...
				if get_product_count > 0:
					...
					if get_product_meta_count > 0:
						...
						image = "https://d1lwvo1ffrod0a.cloudfront.net/41191/-original-imagcv74bakukjvh.jpeg"
