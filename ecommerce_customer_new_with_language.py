...
ecommerce_customer_new_with_language = Blueprint('ecommerce_customer_new_with_language_api', __name__)
api = Api(ecommerce_customer_new_with_language,  title='Ecommerce API',description='Ecommerce API')
name_space = api.namespace('EcommerceCustomerNewWithLanguage',description='Ecommerce Customer')
...
@name_space.route("/CheckEmail/<int:organisation_id>")	
class CheckEmail(Resource):
	...
	def post(self,organisation_id):		
		...
		getData = (details['email'],organisation_id)
...
@name_space.route("/CheckPhoneno/<int:organisation_id>")	
...
@name_space.route("/getLocation/<int:organisation_id>")	
...
#----------------------Dashboard---------------------#
@name_space.route("/dashboard/<int:category_id>/<int:user_id>/<int:organisation_id>/<string:language>")	
class dashboard(Resource):
	def get(self,category_id,user_id,organisation_id,language):
		...
		get_top_selling_product =  ("""SELECT p.`product_id`,p.`product_name`,pm.`product_meta_id`
			...
			INNER JOIN `product` p ON pm.`product_id` = p.`product_id` 
			WHERE pts.`organisation_id` = %s and p.`status` = %s and pts.`language` = %s
			LIMIT 6""")	
		...
		get_best_selling_product =  ("""SELECT p.`product_id`,p.`product_name`,pm.`product_meta_id`,pm.`out_price` price
			...
			INNER JOIN `product` p ON pm.`product_id` = p.`product_id` 
			WHERE pbsm.`organisation_id` = %s and p.`status` = %s and pbsm.`language` = %s
			LIMIT 6""")
		...
		get_offer_product =  ("""SELECT pom.`product_id`,o.`offer_id`,o.`offer_image`,o.`discount_percentage`
			...
			WHERE pom.`organisation_id` = %s and pom.`language` = %s""")
		...
		get_new_arrival_product =  ("""SELECT pnm.`product_id`,n.`new_arrival_id`,n.`new_arrival_image` as `offer_image`,n.`discount_percentage`,n.`image_type`
			...
			WHERE pnm.`organisation_id` = %s and pnm.`language` = %s
			limit 6""")
...
@name_space.route("/getProductListByCategory/<int:category_id>/<int:organisation_id>/<string:language>")	
class getProductListByCategory(Resource):
	def get(self,category_id,organisation_id,language):
		...
		get_latest_product =  ("""SELECT p.`product_id`,p.`product_name`,pm.`product_meta_id`,pm.`out_price`,pm.`loyalty_points`
			...
			INNER JOIN `product_meta` pm ON pbsm.`product_meta_id` = pm.`product_meta_id`
			INNER JOIN `product` p ON pm.`product_id` = p.`product_id` 
			WHERE p.`category_id` = %s and pbsm.`organisation_id` = %s and pbsm.`language` = %s and p.`status` = %s
			LIMIT 6""")
		product_status = 1
