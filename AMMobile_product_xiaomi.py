...
AMMobile_product_xiaomi = Blueprint('AMMobile_product_Xiaomi', __name__)
api = Api(AMMobile_product_xiaomi,  title='AM Mobile Product Xiaomi',description='AM Mobile Product Xiaomi API')
name_space = api.namespace('AMMobileProductXiaomi',description='AM Mobile Product Xiaomi')
	...
	def post(self):
		...
		if brand == 'Xiaomi' or brand == 'Mi':			
			try:
				...
				get_product_name = get_product_name.replace(" Ram", "", 1).rstrip()
					
				if " Redimi " in get_product_name or " Mi " in 	get_product_name or "MI":
					get_product_name = get_product_name.replace(" Redimi ", " ", 1)
				...
				if brand == 'Xiaomi':
					...
					if "(" in get_product_name:
						...
						new_ram = re.sub("[A-Za-z]+", lambda ele: " " + ele[0] + "", ram)
						remove_storage_ram_string = "("+ram+" "+storage+")"
						print(remove_storage_ram_string)
						product_name = get_product_name.replace(remove_storage_ram_string,"",1).rstrip()					
					else:										
						product_name_split_for_storage_ram = get_product_name.split(' ')
						storage = product_name_split_for_storage_ram[-1]						
						ram = product_name_split_for_storage_ram[-2]
						new_storage =  re.sub("[A-Za-z]+", lambda ele: " " + ele[0] + "", storage)
						new_ram = re.sub("[A-Za-z]+", lambda ele: " " + ele[0] + "", ram)
						remove_storage_ram_string = ram+" "+storage
						product_name = get_product_name.replace(remove_storage_ram_string,"",1).rstrip()
				...
				if get_product_count > 0:
					...
					if get_product_meta_count > 0:
						...
						image = "https://d1lwvo1ffrod0a.cloudfront.net/40911/mi-redmi-9a-b08696xb4b-original-imafve7pjpemhwk5.jpeg"
...
@name_space.route("/getProductNameAndMetaTest")
class getProductNameAndMetaTest(Resource):
	@api.expect(Product_Name_postmodel)
	def post(self):
		connection = mysql_connection_ecommerce()
		cursor = connection.cursor()
