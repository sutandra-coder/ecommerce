...
AMMobile_product_realme = Blueprint('AMMobile_product_realme_api', __name__)
api = Api(AMMobile_product_realme,  title='AM Mobile Product Realme',description='AM Mobile Product Realme API')
name_space = api.namespace('AMMobileProductRealme',description='AM Mobile Product Realme')
...
class getProductNameAndMeta(Resource):
	...
	def post(self):
		...
		if brand == 'Realme':
			...
			try:
				brand_id = 10
				if "(2021)" in get_product_name:
					get_product_name = get_product_name.replace(" (2021)", "", 1).rstrip()
				else:
				...
				RMX = new_product_name_split[-1]

				if "RMX" in RMX:
					new_product_name = get_product_name.replace(RMX, "",1).rstrip()
				else:
					new_product_name = get_product_name			

				product_name_split_for_storage_ram = new_product_name.split(' ')
				storage_ram_string = product_name_split_for_storage_ram[-1]
				final_product_name = new_product_name.replace(storage_ram_string, "",1).rstrip()
				...
				if get_product_count > 0:
					...
					if get_product_meta_count > 0:
						...
						image = "https://d1lwvo1ffrod0a.cloudfront.net/28/realme_default.jpeg"
